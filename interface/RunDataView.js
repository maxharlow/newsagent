...
export default class RunDataView extends React.Component {
    ...
    render() {
        ...
        const count = HTML.span({ className: 'count' }, countText)
        if (this.state[this.state.mode + 'Checking']) {
            const doLoad = () => this.setState({ [this.state.mode + 'Checking']: false })
            const load = HTML.button({ onClick: doLoad }, 'Load anyway?')
            const message = HTML.span({ className: 'message' }, 'This data is very large!', load)
            const data = HTML.div({ className: 'data' }, message)
            const box = HTML.div({ onClick: e => e.stopPropagation() }, closeButton, title, data, ...modes, count)
            return HTML.div({ className: 'run-data-view', ref: node => this.node = node, onClick: this.props.close }, box)
        }
        else if (this.state[this.state.mode] === null) {
            const loading = HTML.div({ className: 'loading' })
            const data = HTML.div({ className: 'data' }, loading)
            const box = HTML.div({ onClick: e => e.stopPropagation() }, closeButton, title, data, ...modes, count)
            return HTML.div({ className: 'run-data-view', ref: node => this.node = node, onClick: this.props.close }, box)
        }
        else if (this.state[this.state.mode].length === 0) {
            const message = HTML.span({ className: 'message' }, 'No data found.')
            const data = HTML.div({ className: 'data' }, message)
            const box = HTML.div({ onClick: e => e.stopPropagation() }, closeButton, title, data, ...modes, count)
            return HTML.div({ className: 'run-data-view', ref: node => this.node = node, onClick: this.props.close }, box)
        }
            ...
            const data = HTML.div({ className: 'data' }, table)
            const box = HTML.div({ onClick: e => e.stopPropagation() }, closeButton, title, data, ...modes, count)
            return HTML.div({ className: 'run-data-view', ref: node => this.node = node, onClick: this.props.close }, box)
        }
    }
