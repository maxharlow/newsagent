...
export default class AgentPageRecipe extends React.Component {
    ...
    render() {
        const elements = [
            React.DOM.h3({}, 'Recipe'),
            React.DOM.hr({}),
            React.DOM.p({}, 'Runs at ' + PrettyCron.toString(this.props.recipe.schedule).toLowerCase() + (this.props.state === 'running' ? '. Next run is ' + PrettyCron.getNext(this.props.recipe.schedule).toLowerCase() : '') + '.'),
            React.DOM.h4({}, 'Setup'),
            ...
            React.DOM.h4({}, 'Result'),
            React.DOM.p({ className: 'field' }, this.props.recipe.result),
