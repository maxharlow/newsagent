...
export function listen() {
    ...
    app.get('/agents/:id', (request, response) => {
        Agents.get(request.params.id)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    ...
    app.get('/agents/:id/build', (request, response) => {
        Database.retrieve('build', request.params.id)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent build not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    app.get('/agents/:agent/runs', (request, response) => {
        Agents.getRuns(request.params.agent)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent runs not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    app.get('/agents/:agent/runs/:run', (request, response) => {
        Agents.getRun(request.params.agent, request.params.run)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent run not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    app.get('/agents/:agent/runs/:run/execution', (request, response) => {
        Agents.getRunExecution(request.params.agent, request.params.run)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent execution not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    app.get('/agents/:agent/runs/:run/data', (request, response) => {
        ...
        Agents.getRunData(request.params.agent, request.params.run, asCSV)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent data not found' })
                else response.status(500).send({ error: e.message })
            })
    })
    app.get('/agents/:agent/runs/:run/data/added', (request, response) => {
        ...
        Agents.getDiffAdded(request.params.agent, request.params.run, asCSV)
            ...
            .catch(e => {
                if (e.message && e.message.contains('404')) response.status(404).send({ error: 'agent diff not found' })
                else response.status(500).send({ error: e.message })
            })
    })
