...
async function buildContext(client, id, recipe) {
    const packages = [ 'build-base', 'git', 'curl', 'wget', 'bash', 'python2', 'python2-dev', 'py2-pip', 'python3', 'python3-dev', 'ruby', 'ruby-dev', 'nodejs-current' ]
...
async function fromContainer(id, method, path, data) {
    ...
    const client = await Docker.client(agent.client)
    const container = await client.getContainer(id)
    const command = ['curl', '-X', method, '-H', 'Content-Type: application/json', '-m', '60']
